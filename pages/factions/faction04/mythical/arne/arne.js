    ...
    spells: [
        {
          ...
          name: "Chant d'acier",
          description: `Attaque un ennemi. Place une frappe supplémentaire si la cible se trouve sous débuff ${DEBUFFS.DEATHBRAND}.${RETURN}${RETURN}
...
Réduit les PV et la DÉF de la cible de 10 % (s'accumule jusqu'à 30 %). Cet effet ne fonctionne pas contre les Boss.`,
        ...
        },
        {
          ...
          description: `Attaque un ennemi. Avant d'attaquer, place un débuff ${DEBUFFS.LOCK_PASSIF} pendant 1 tour. Ignorera 50 % de la DÉF de la cible, ainsi que les buffs ${BUFFS.BLOCK_DAMAGE} et ${BUFFS.UNKILLABLE}.${RETURN}${RETURN}
...
Si la cible initiale est tuée, répétera l'attaque contre tous les ennemis et ignorera 25 % de la DÉF de chaque cible. Réduira également de 2 tours le temps de recharge de la compétence Hurlement polaire si la cible initiale est tuée lorsqu'elle se trouve sous débuff ${DEBUFFS.DEATHBRAND}.`,
        ...
        },
        {
          ...
          description: `Place un buff ${BUFFS.ATK} de 50 % et un buff ${BUFFS.PRE} de 50 % sur tous les alliés pendant 3 tours.${RETURN}${RETURN}

Place un débuff ${DEBUFFS.DEATHBRAND} sur tous les ennemis pendant 2 tours.${RETURN}${RETURN}
...
Ce Champion est immunisé contre les débuffs ${DEBUFFS.GEL}, ${DEBUFFS.STUN} et ${DEBUFFS.PROVOKE}.${RETURN}${RETURN}
...
Si les PV de MAX de ce Champion sont supérieurs ou égaux à ceux de la cible, il est impossible de résister à ce débuff.`,
        ...
        },
        {
          ...
          description: `Place un débuff ${DEBUFFS.PROVOKE} sur tous les ennemis pendant 1 tour. Si la cible est un Boss, place au lieu de ça un débuff ${DEBUFFS.PROVOKE} pendant 2 tours. Si les PV MAX de ce Champion sont supérieurs ou égaux à ceux de la cible, il est impossible de résister à ces débuffs.${RETURN}${RETURN}
...
Chaque fois qu'un Champion est tué, augmente de 20 % les PV et la DÉF de ce Champion (s'accumule jusqu'à 100 %).${RETURN}${RETURN}

Immunisé contre les débuffs ${DEBUFFS.GEL}, ${DEBUFFS.STUN} et ${DEBUFFS.PROVOKE}.${RETURN}${RETURN}
