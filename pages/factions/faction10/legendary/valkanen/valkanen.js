  ...
  spells: [
        {
          ...
          name: "Faux d'effroi",
          description: `Attaque un ennemi. Cette attaque ignore les buffs ${BUFFS.SHIELD}.${RETURN}${RETURN}

Possède 60 % de chances d'augmenter d'1 tour la durée de tous les débuffs de la cible.${RETURN}${RETURN}

Possède également 80 % de chances d'appliquer un effet de ${PROP_DEBUFF} qui prend 2 débuffs aléatoires de la cible pour les placer sur tous les ennemis sous débuff ${DEBUFFS.HEX}.`,
          damage: "3.6*ATQ",
...
Avant d'attaquer, possède 80 % de chances de placer un débuff ${DEBUFFS.HEX} sur tous les ennemis pendant 2 tours. Ignorera 20 % de la RÉS de chaque ennemi pour chaque allié mort.${RETURN}${RETURN}
...
Si l'allié est en vie, le tue et débloque une compétence secrète, Déchaînement de malice. Place également un débuff ${DEBUFFS.WEAKEN} de 25 % pendant 2 tours et un débuff ${DEBUFFS.TRUE_FEAR} pendant 1 tour sur tous les ennemis. Il est impossible de résister à ces débuffs ou de les bloquer. Ensuite, remplit le Compteur de Tour de ce Champion de 75 %.${RETURN}${RETURN}
...
Avant d'attaquer, vole tous les buffs de la cible. Il est impossible de résister à cet effet.${RETURN}${RETURN}

Si la cible possède une ATQ supérieure ou égale à celle de ce Champion, cette attaque gagne un multiplicateur de dégâts bonus égal à l'ATQ de la cible. Non applicable aux Boss.${RETURN}${RETURN}

Si l'ATQ de la cible est inférieure à celle de ce Champion, cette attaque ignorera 50 % de la DÉF de la cible.${RETURN}${RETURN}
...
Place un buff ${BUFFS.SHIELD} sur ce Champion chaque fois qu'un allié ou qu'un Champion ennemi meurt. La valeur du ${BUFFS.SHIELD} est égale à 50 % des PV MAX du Champion mort. Il est impossible de retirer, de voler ou de transférer ce buff ${BUFFS.SHIELD}.${RETURN}${RETURN}

Si ce Champion se trouve déjà sous buff ${BUFFS.SHIELD}, la valeur du ${BUFFS.SHIELD} est augmentée de 50 % des PV MAX du Champion mort. Ne s'active pas quand un Boss ou un sbire meurt.${RETURN}${RETURN}

${PASSIVE} 

Lorsqu'un ennemi frappe ce Champion alors qu'il se trouve sous buff ${BUFFS.SHIELD}, place un débuff aléatoire sur l'ennemi qui attaque. Les débuffs ${DEBUFFS.TRUE_FEAR}, ${DEBUFFS.GEL}, ${DEBUFFS.PROVOKE}, ${DEBUFFS.PETRIFICATION}, ${DEBUFFS.SLEEP} et ${DEBUFFS.STUN} sont placés pendant 1 tour. Tous les autres débuffs sont placés pendant 2 tours. Ignorera 20 % de la RÉS ennemie pour chaque allié mort.${RETURN}${RETURN}
